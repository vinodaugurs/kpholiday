...
class Agent extends CI_Controller {
    ...
    public function show_package($id) {
        ...
        $data['display'] = $this->region_model->get_data('packages', array('pack_id' => $id));
        $this->load->view('agent/detail-page', $data);
    }
...
//forget password start
    ...
    public function OnewayFlightBooking($UserTrackId) {
        ...
        if ($this->input->post('flight') == 'confirm') {
            ...
            $cdata['uid'] = $this->session->userdata('uid');
            $cdata['commission'] = $this->session->userdata('flight_ticket_commission');
            
            $GetBook = $this->CutlPost(base_url('/index.php/flight/ManageOnewaybookingAgent'), $cdata);
    ...
    public function IntOnewayFlightBooking($UserTrackId) {
        ...
        if ($this->input->post('flight') == 'confirm') {
            ...
            //print_r($cdata);
            $GetBook = $this->CutlPost(base_url('/index.php/flight/International_ManageOnewaybookingAgent'), $cdata);
    ...
    public function bus_getGetCancellation() {
        ...
        if ($result['ResponseStatus']) {
            if (isset($result['CancellationOutput']['CanceledTicketDetails']['CanceledPNRDetails'])) {
                ...
                if ($this->user_model->update_table_data('bus_bookings', $condition)) {
                    $this->load->model('sendmail_model');
                    unset($payData['uid']);
                    $data['msgData'] = $payData;
                    $msg = $this->load->view('email/customer', $data, true);
                    $email = $this->session->userdata('aemail');
                    $this->sendmail_model->send_mail2("no-reply@kpholidays.com", "KP Holidays", $email, 'KP Holidays Fight Booking', $msg);
                }
    ...
    public function hotel_thankyou($BookingID = NULL) {
        ...
        $url = base_url('/index.php/hotel/print_dom_ticket/' . $BookingID . '/true');
        $ch = curl_init();
        ...
        $thankyou['data'] = json_decode($msg, true);
    ...
    public function hotel_thankyouIntl($BookingID = NULL) {
        ...
        $msg = "";
        $headers = array('Content-Type: application/json');
        $url = base_url('/index.php/hotel/print_Intl_ticket/' . $BookingID . '/true');
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, FALSE);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    ...
    public function hotel_getGetCancellation() {
        ...
        if ($this->input->post('HotelType') == 'Domestic') {
            ...
            if (isset($response['soapBody']['HotelBookingCancelRQResponse']['HotelBookingCancelRQResult']['CancelBooking']['CancelDetails']['Cancel']['@attributes']['CancelStatus'])) {
                ...
                if ($this->user_model->update_table_data('hotel_bookings', $condition)) {
                    $this->load->model('sendmail_model');
                    unset($payData['uid']);
                    $data['msgData'] = $payData;
                    $msg = $this->load->view('email/customer', $data, true);
    ...
    public function IntOnewayThankyou($tranctionid, $UserTrackId) {
        ...
        $msg = "";
        $headers = array('Content-Type: application/json');
        $url = base_url('/index.php/flight/IntOnewayThankyou/' . $tranctionid . '/' . $UserTrackId . '/true');
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, FALSE);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    ...
    public function OnewayThankyou($tranctionid, $UserTrackId) {
        ...
        $msg = "";
        $headers = array('Content-Type: application/json');
        $url = base_url('/index.php/flight/OnewayThankyou/' . $tranctionid . '/' . $UserTrackId . '/true');
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, FALSE);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    ...
    public function hotel_bookingIntl($HotelId = NULL, $Bookingkey = NULL, $TrackId = NULL) {
        ...
        if ($this->input->post('booking') == 'confirm') {
            ...
            $tdata['uid'] = $this->session->userdata('uid');
            $tdata['commission'] = $this->session->userdata('hotel_commission' . $Bookingkey);
    ...
    public function CutlPost($url, $postdata) {
    ...
    public function airline_ticket_history($offset = 0) {
        ...
        if (!empty($urlData)) {
            ...
            $config['base_url'] = base_url('index.php/agent/airline_ticket_history');
        }
        $data = array();
    ...
    public function walletReporting($offset = 0) {
        ...
        if (!empty($urlData)) {
            ...
            $config['base_url'] = base_url('index.php/agent/walletReporting');
        }
        $data = array();
        if (isset($_GET['submit'])) {
    ...
    public function bus_ticket_history($offset = 0) {
        ...
        if (!empty($urlData)) {
            ...
            $config['base_url'] = base_url('index.php/agent/bus_ticket_history');
        }
        $data = array();
        if (isset($_GET['submit'])) {
    ...
    public function hotel_ticket_history($offset = 0) {
        ...
        if (!empty($urlData)) {
            ...
            $config['base_url'] = base_url('index.php/agent/hotel_ticket_history');
        }
        $data = array();
        if (isset($_GET['submit'])) {
    ...
    public function flight_getGetCancellation() {
        ...
        if ($this->input->post('action')) {
            ...
            if ($result['ResponseStatus']) {
                ...
                if ($this->user_model->add_data('canceled_tickets', $payData)) {
                    ...
                    $msg = $this->load->view('email/customer', $data, true);
                    $email = $this->session->userdata('aemail');
                    $this->sendmail_model->send_mail2("no-reply@kpholidays.com", "KP Holidays", $email, 'KP Holidays Fight Booking Cancel', $msg);
                }
    ...
    public function edit_markup() {
        ...
        if (isset($_POST['updatemarkup']) and $this->input->post('updatemarkup') == true and ( $this->input->post('forcommision') == 'International Flight' or $this->input->post('forcommision') == 'Domestic Flight')) {
            if ($this->input->post('markupid') == '') {
                $flights = $this->user_model->add_data('agent_commision_markup', array('uid' => $this->session->userdata('uid'),
                ...
                $flights = $this->user_model->update_data('agent_commision_markup', array(
                    'adult_Commission' => $this->input->post('Adult_commision'),
                    'Child_Commission' => $this->input->post('Child_Commission'),
                    'Infant_Commission' => $this->input->post('Infant_Commission')
        ...
        if ($markuptype == "International Flight" or $markuptype == "Domestic Flight") {
            ...
            if (isset($_POST['updatemarkup']) and $this->input->post('updatemarkup') == true) {
                if ($this->input->post('markupid') == '') {
                    $this->user_model->add_data('agent_commision_markup', array('uid' => $this->session->userdata('uid'),
                    ...
                    $this->user_model->update_data('agent_commision_markup', array(
