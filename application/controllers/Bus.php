  ...
  public  function GetBookAgent()
   {
	   $data=$_POST;
	   		...
	   		//$data['TotalAmount'] = $data['TotalAmount']-$commission;
		...
		if($result['ResponseStatus']){
					...
					// $tdata['commission_amount'] =  $data['commission'];
					$tdata['user_id']=$data['auid'];
					$tdata['AGENTCODE']=$data['auid'];
					$tdata['GIREFID']=$result['UserTrackId'];
					$tdata['METHODID']="BusBooking";
					$tdata['BALANCE']=($balance-$data['TotalAmount']-$commission);
					$tdata['TOTALAMOUNT']=trim($balance);
					$tdata['DEBITAMOUNT']=trim($data['TotalAmount']-$commission);
					$tdata['RECHARGEFEE']=0;
					...
					$this->user_model->user_wallet_amout_update(($balance-$data['TotalAmount']-$commission),$data['auid']);	
  ...
  function savePuMoney($payData){
	...
	function saveBooking($data,$bookingdetails){	 
	  		 ...
	  		 // print_r($bookingdetails);
				...
				$payData['Totalmarkup']=(isset($bookingdetails['Totalmarkup']) and $bookingdetails['Totalmarkup']!='')?$bookingdetails['Totalmarkup']:0;
				$payData['status']='CONFIRM';
				$payData['commission']=isset($bookingdetails['commission'])?$bookingdetails['commission']:'User booking';
				...
				$tranctionid = $payData['TransactionId'];

				$msg=file_get_contents(base_url("/index.php/bus/get_print1/$tranctionid"));
				...
				$file=$pdf_file=$this->pdf->createPdf($msg);
				
				$this->sendmail_model->send_ticket_mail("no-reply@kpholidays.com","KP Holidays",$email,'KP Holidays Bus Booking',$msg ,$file);
				//send sms
				$this->load->library('sms');
				 $msg = 'Your Bus ticket booking has been confirmed. Your Transport PNR: '.$payData['TransportPNR'].', Total Ticket: '.$payData['TotalTickets'].', Origin: '.$payData['Origin'].', Destination: '.$payData['Destination'].'. Please Check your email for more details. -KP Holidays.com';
  	...
  	//$this->load->library('sms');
	  	...
	  	$this->load->library('pdf');
				...
				//$email='rahulpal@augurs.in';
				 //$this->sendmail_model->send_ticket_mail("no-reply@kpholidays.com","KP Holidays",$email,'KP Holidays Bus Booking',$msg ,'http://kpholidays.com/assets/img/Final-logo.png');
				//$mobile= "+918090808989";
				//$msg='bus ticket';
				//$image = dirname(__FILE__) . 'test.pdf';
				//echo $image;
//				echo "send mail";
				echo $data = $this->pdf->createPdf($msg);
		//$test = $this->pdf->createPdf($msg);
  ...
  function CutlPost($url,$postdata){
	 ...
	 if(!$this->input->post('TicketNumber')){
		...
		if($result['ResponseStatus']){
			if(isset($result['CancellationOutput']['CanceledTicketDetails']['CanceledPNRDetails'])){
				...
				$msg=$this->load->view('email/cancel',$data,true);
				$email=$this->session->userdata('email');
				$this->sendmail_model->send_ticket_mail("no-reply@kpholidays.com","KP Holidays",$email,'KP Holidays Bus Ticket Cancel',$msg);
				...
				$msg1 = "Your Bus ticket booking has been confirmed. Your Transport PNR: ".$payData['HermesPNR'].", Ticket Cancellation Number:".$payData['AirlinePNR']." Please Check your email for more details. -KP Holidays.com";
				$mobile=$data['0']['phone'];
	...
	public function get_print($tranctionid){
	  	...
	  	$dataTable = $this->user_model->get_data('bus_bookings', $condition);		
	  	$ScheduleId = $this->session->userdata('ScheduleId');
	  	$bus_dietail=$this->session->userdata('Bus_'.$ScheduleId);
	  	$result=$this->GetReprint($tranctionid);
   	    $data['data']=json_decode($result,true);
	    $data['bus_dietail']=$bus_dietail;
		$data['Totalmarkup']=$dataTable[0]['Totalmarkup'];
	...
	public function get_print1($tranctionid){
	  	$condition = array('TransactionId'=>$tranctionid);
	  	$dataTable = $this->user_model->get_data('bus_bookings', $condition);		
	  	$ScheduleId = $this->session->userdata('ScheduleId');
	  	$bus_dietail=$this->session->userdata('Bus_'.$ScheduleId);
	    ...
	    $data['status'] = $this->user_model->ticket_status($tranctionid);
	  	...
	  	$printDetail = array('full_detail'=>$data);	  	
	  	$this->load->view('user/print-bus-ticket1',$printDetail);
	}
